...
from common.shared import DateTransfer, QuestionStr,CitySearch
from dialogue import SQLAlchemyDB as db
...
class FlightInfo(db.Model):
    ...
    def __get_flight_info(self, text):
        times = []
        locations = []
        cancel_tuple = (u"取消", u"删除")
        confirm_tuple = (u"是", u"是的")
        result = self.nlp.ner(text)[0]
        words = result['word']
        entities = result['entity']

        for i in range(0, len(words)):
            if words[i] in cancel_tuple:
                self.cancel_status = 1
                return
            if self.status == 7 and words[i] in confirm_tuple:
                self.cancel_status = 2
                return
        ...
        for entity in entities:
            if entity[2] == "time":
                texttime = words[entity[0]:entity[1]]
                formattime = self.nlp.convert_time(texttime, datetime.datetime.today())
                times.append(formattime['timestamp'].split()[0])
    ...
    def __change_status(self):
        if self.cancel_status == 1:
            self.status = 7
            return
        if self.cancel_status == 2:
            self.status = 8
            self.complete = True
            return
